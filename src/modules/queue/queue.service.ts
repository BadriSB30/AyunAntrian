...
export class QueueService {
	...
	static async create(data: CreateQueueDTO): Promise<QueueResponse> {
		...
		// Generate nomor antrian: {KODE_LOKET}-{KODE_SHIFT}-{NNN}
		// Reset otomatis saat shift_id berbeda
		const nomorAntrian = await QueueRepository.getNextQueueNumber(data.counter_id, data.shift_id);
	...
	static async update(id: number, data: UpdateQueueDTO): Promise<void> {
		if (!data.status) throw new HttpError(400, 'Status is required');

		const queue = await QueueRepository.findById(id);
		if (!queue) throw new HttpError(404, 'Queue not found');

		const now = new Date();
		const payload: Partial<QueueEntity> = { status: data.status };
